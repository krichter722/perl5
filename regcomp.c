    ...
    )
{
  ...
  * are already native, so no translation is done. */
    ...
    if (endbrace - RExC_parse < 2 || ! strBEGINs(RExC_parse, "U+")) {
         ...
         * different each time, which could cause us to write out-of-bounds if
         * the pass1 name is shorter than the one returned for pass2. */
        ...
        /* Count the the code points, if caller desires.  The API says to do
         * this even if we will later return FALSE */
        if (cp_count) {
            *cp_count = 0;
     ...
     * stored in 'substitute_parse'  In the case of a single-quoted pattern,
     * the string is the bytes to parse; otherwise it is a series of
     * valid "\x{...}\x{...}" constructs. */
    save_start = RExC_start;
    orig_end = RExC_end;
    save_strict = RExC_strict;
